...
import { SIGNUP_ROLES, signUpAccount, signupNextForRole, type SignupRole } from "@/lib/auth/flow";
...
export default function SignupForm({ supabaseReady, next = "/account" }: { supabaseReady: boolean; next?: string }) {
  ...
  const roleNext = signupNextForRole(role, next);
  ...
  async function resend() {
    ...
    try {
      ...
      const { error } = await client.auth.resend({ type: "signup", email: email.trim(), options: { emailRedirectTo: `${window.location.origin}/auth/callback?next=${encodeURIComponent(roleNext)}` } });
  ...
  if (confirmation) return <div className="mt-8 space-y-4">
    ...
    <p className="text-sm text-text-secondary">Already registered? <Link className="text-accent hover:underline" href={`/auth/login?next=${encodeURIComponent(roleNext)}`}>Log in</Link> or <Link className="text-accent hover:underline" href="/auth/forgot-password">reset your password</Link>.</p>
  ...
  return <form className="mt-8 space-y-4" onSubmit={onSubmit}>
    ...
    <p className="text-sm text-text-muted">After signup you’ll land on {roleNext === "/tutorial/player" ? "the player walkthrough" : roleNext === "/tutorial/business" ? "the business walkthrough" : roleNext === "/discover" ? "Discover" : "your next step"} — skippable anytime.</p>
