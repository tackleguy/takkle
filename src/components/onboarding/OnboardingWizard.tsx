  ...
  return <div className="mx-auto max-w-2xl rounded-xl border border-border bg-bg-card p-5 sm:p-8">
    {submitted ? <div role="status">
      ...
      {selected && <div className="mt-6 border-t border-border pt-6">
        ...
        {!available ? <p className="mt-4 text-sm text-text-secondary">Claim submissions are temporarily unavailable. You can still browse player profiles.</p> : !signedIn ? <div className="mt-4"><p className="mb-3 text-sm text-text-secondary">Sign in to securely submit and track your claim.</p><Button href={`/auth/login?next=${encodeURIComponent(next)}`}>Sign in to claim</Button><Link href="/auth/signup" className="ml-4 text-accent hover:underline">Create account</Link></div> :
