...
import { fetchOpsmaAllOhio } from "./opsma-allohio";
import { fetchCfbdRecruitsForStates } from "../shared/cfbd-state";
import type { StateAdapterResult } from "../types";

export async function runOhioAdapter(options?: {
  rootDir?: string;
  schoolLimitPerState?: number;
}): Promise<StateAdapterResult> {
  const rootDir = options?.rootDir ?? process.cwd();
  const summary = emptyRunSummary("ohio", "OH");
  const schools = loadNcesSchools(rootDir, ["OH"], options?.schoolLimitPerState);
  summary.schoolsDiscovered = schools.length;

  const { players, errors, blocked } = await fetchOpsmaAllOhio({ rootDir });
  summary.errors.push(...errors);
  summary.blockedSources.push(...blocked);
  summary.blockedSources.push(
    "OHSAA portal paths under /Portals/ Disallow — honor-roll PDF/news used instead of full rosters.",
  );

  const cfbd = await fetchCfbdRecruitsForStates(["OH"], { rootDir });
